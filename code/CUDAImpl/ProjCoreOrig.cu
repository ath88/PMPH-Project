...
__global__ void rollback1_kernel(unsigned int g, PrivGlobs &globs) {
	...
	for(int i=0; i<numX; i++) {
		...
		int myResultId = i*globs.numY + j;
		REAL myResultSub[3] = {
				myResult[myResultId - 1],
				myResult[myResultId],
				myResult[myResultId + 1],
		};
		...
		if(j > 0) {
			v[i*numY + j] += (0.5
					* myVarY * globs.myDyy[j*4 + 0])
					* myResultSub[0];
		}
		v[i*numY + j] += (0.5
				* myVarY * globs.myDyy[j*4 + 1])
				* myResultSub[1];
		if(j < numY - 1) {
			v[i*numY + j] += (0.5
...
void rollback0_host (unsigned int g, PrivGlobs &globs) {
	...
	rollback0_kernel <<< blocks, threads, sizeof(REAL)*globs.numY*4 >>> (g, *globs.d_globs);
	cudaDeviceSynchronize();
