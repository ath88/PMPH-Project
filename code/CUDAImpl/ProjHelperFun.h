...
struct PrivGlobs {
	...
	void copyToDevice() {
		...
		cudaMemcpy(this->device->myVarX, this->myVarX, sizeof(REAL) * outer * numX * numY, cudaMemcpyHostToDevice);
		cudaMemcpy(this->device->myVarY, this->myVarY, sizeof(REAL) * outer * numX * numY, cudaMemcpyHostToDevice);
