...
This will help us determince what we definitely should parallelize, and what is
less relevant. 
...
This means that all iterations are distributed during compliation. This gives
less overhead, compared to dynamic scheduling, which happens at runtime. We do
this because we know that each iteration of the outer loop yields the same
amount of work, such that no iteration takes much longer time that the others.
Had they been of different size, then static scheduling would lead to load 
imbalance, and incur extra overhead due to siginificant idling on some cores.
...
did, and in approximately chronological order.
...
need to keep the struct available across multiple CUDA kernels.
